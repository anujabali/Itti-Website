-- Use the acquisition columns, and stop trusting `heardFrom` blindly.
--
-- `heard_from`, `heard_from_other`, `referrer_name`, `referrer_code`, `utm` and
-- `contact_preferred` have been on `person` since 0001, accepted by this
-- function since 0004, and never once written to: the form asked for none of
-- them. 0001's own note says why they exist — "self-report and UTM both,
-- because each catches what the other misses: UTM is accurate but blind to
-- offline word of mouth."
--
-- The form now asks how someone heard of us, and the page's campaign tags and
-- referring host travel with the registration. Two changes are needed here:
--
--   1. `heardFrom` was cast straight to its enum with nothing checking it, so
--      an unexpected value raised rather than answering. Every other enum in
--      this function is checked; this one was missed.
--   2. `contact_preferred` is derived rather than asked. Three separate consent
--      boxes already say how someone may be reached; a fourth question asking
--      which they prefer is the same question again. Exactly one permission is
--      a preference, more than one states none.
