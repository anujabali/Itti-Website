-- A guardian reachable only by phone was not recorded at all.
--
-- 0013 wrote a guardian_notice only when there was an email address to send it
-- to. But the form accepts a phone number instead, and the privacy page
-- undertakes that somebody contacts the guardian before a minor's registration
-- is acted on. With no row and no SMS sender, that undertaking had nothing
-- behind it and nothing to show it was missing.
--
-- The row is written either way now. Without an email it is `skipped`, with the
-- reason on it, which makes it a number somebody can look at:
--
--   select count(*) from notification
--   where kind = 'guardian_notice' and status = 'skipped';
