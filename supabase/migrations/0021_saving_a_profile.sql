-- Saving a profile
--
-- The same function, widened to take the bio and the links 0019 made room for.
-- Everything it already validated is untouched; what is new is checked before
-- anything at all is written, so a mistyped handle cannot leave half a profile
-- saved and half refused.
...
comment on function update_my_registration (jsonb) is
  'Writes back the fields a person may correct about themselves, including their own bio and links. Email, guardian, acquisition and the binding itself are not among them.';
