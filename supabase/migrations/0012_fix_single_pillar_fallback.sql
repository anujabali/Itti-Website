-- The single-value fallback in 0011 never ran.
--
-- It was written as a coalesce over `array(select jsonb_array_elements_text(...))`,
-- and that expression returns an empty array when the key is absent, not null —
-- so coalesce always took the first branch and `selectedPillar` was ignored.
--
-- This matters immediately: the deployed browser bundle still sends the single
-- value, so between 0011 and this, a registration from the live site recorded
