...
export const COUNTRIES: Country[] = Array.from(
	new Map(
		raw.map(([iso, name, dial, min, max]) => [
			iso,
			{ iso2: iso, name, dialCode: dial, flag: flag(iso), min, max },
		]),
	).values(),
);
...
export function searchCountries(q: string) {
	...
	return COUNTRIES.filter(
		(c) =>
			...
			c.iso2.toLowerCase() === x ||
			c.dialCode.includes(x.replace(/^\+/, '').trim()),
	);
}
export function detectCountryFromInternationalNumber(value: string): Country | null {
	...
	return (
		[...COUNTRIES]
			...
			.find((c) => normalized.startsWith(c.dialCode.replace('+', ''))) || null
	);
}
